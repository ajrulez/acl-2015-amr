...
SRL++ is \textit{not} the hard part of AMR parsing. The hard part of AMR parsing is NER++. The evidence for this claim comes from a close reading of the only published AMR parser to date, \newcite{2014flanigan-amr}, dubbed ``JAMR''.
...
We show that this approach improves end-to-end recall dramatically over previous approaches, and that end to end performance is improved from 0.59 to 0.62 \n{smatch} (the metric for measure AMR parsing performance, \cite{cai2013smatch-amr}) when our generative actions are stitched together by the previous state of the art parser \cite{2014flanigan-amr}.
Our system is able to correctly handle 77\% of tokens in the dev set, where the naive baseline previously in place can only handle 63\%, a gain of 14\%.

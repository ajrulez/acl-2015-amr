...
The Abstract Meaning Representation (AMR) is a semantic formalism that represents meaning as a directed graph, where nodes repesent concepts and arcs are relationships between concepts. AMR makes no effort to have a one-to-one correspondence between nodes in a graph and tokens in a source sentence, and so is not a ``semantic dependency'' representation. In fact, AMR will often expand single tokens into large sub-graph elements, or ignore tokens completely. AMR was designed with MT as a possible application, but with an acknowledged bias towards English. It's often convenient to think of AMR as an unambiguous, computational dialect of English we're translating to, rather than a sophisticated form of dependency parsing.

To introduce AMR and its notation, we'll unpack the parse of "He ran to his house in desperation".

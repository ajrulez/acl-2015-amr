...
AMR also possesses distinctive computational challenges from structural properties.
For example, AMR provides no guarantees about projectivity or even acyclicity of its graphs.
...
Our initial assumption (which we suspect would be shared by many researchers with a background in structured prediction) was to assume that the main area deserving further attention in AMR parsing is predicting the edges in the unreliably-aligned, cyclic, non-projective graphs of AMR (i.e., SRL++).
...
For example, we have an action \textit{VERB} that will perform a verb-sense-disambiguation to the appropriate PropBank frame \cite{palmer2005proposition-srl} on the source token, like the \w{ran} to \n{run-01} example in \reffig{glee}.
...
We explain in \refsec{actions} how exactly we manage this partition, and explain in \refsec{data} how we create training data from existing resources to train an action-type classifier, and setup the classifier itself.
...
\paragraph{PERSON} Another common structured data type in AMR is the \n{person} with \n{name} construction. 
...
In our aligner, we bias alignments towards generating more reliable action sequences (see \refsec{align}).
...
\section{Results}
We present improvements in end-to-end AMR parsing given our NER++ component on two datasets,
action type classifier accuracy on an automatically aligned corpus,
and alignment accuracy on a small hand-labeled corpus.

\Subsection{result-amr}{End-to-end AMR Parsing}
We evaluate our novel NER++ component in the context of end-to-end AMR parsing
We evaluate on two corpora: the LDC2014T12 corpus and LDC2013E117 corpus, both consisting
...
AMR parsing accuracy is measured with a metric called smatch \cite{cai2013smatch-amr}, which stands 
  for ``s(emantic) match''. 
...
On a hand-annotated dataset of 100 AMR parses from the LDC2014T12 corpus, our aligner achieves
  an accuracy of \textbf{0.832}. Previous aligners have pursued a slightly different definition of alignment,

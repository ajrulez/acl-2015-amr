...
If a system is considering how to link the node \n{run-01} in \reffig{glee}, the verb-sense frame for ``run-01'' leaves very little entropy in terms of what we could assign as an \w{ARG0} arc.
...
To illustrate, a correct parse of the sentence in \reffig{method} requires lemmatization (\textit{gleefully} $\rightarrow$ \textit{glee}), word sense tagging (\textit{run} $\rightarrow$ \textit{run-01}), and open domain NER (i.e., \textit{Rover}),
...
This paper's main contribution explores delexicalizing NER++ instead, see \refsec{nerplusplus}.
...
This approach derives most of its power from the similarity of English words and their AMR counterparts, and so does not generalize to non-English AMR parsing.

...
In this section we introduce a method for resolving alignment ambiguity based on comparing the reliability with which actions generate the correct sub-graph, and discuss implications.
...
\newcite{jones2012semantics} was focused on the application of AMR to Machine Translation, 
  and worked with a much more limited dataset.
\newcite{2014flanigan-amr} forms the basis of the approach of this paper, and is discussed elsewhere.
...
On a hand-annotated dataset of 100 AMR parses from the LDC2014T12 corpus,\footnote{
    Our dataset is publicly available at \todo{location}
  }
...
In particular, we require that every AMR node aligns to some token in the sentence,
  and force the system to always align nodes, even when unsure.
%EDIT: Error analysis
We find the our informativeness-based alignment objective slightly improves end-to-end performance
  when compared to the rule-based approach of \cite{2014flanigan-amr}, moving up from P=64, R=59, F1=61 to P=65, R=59, F1=62.
...
The classifier's most common class of mistakes are incorrect \textbf{DICT} classifications. It is reassuring that some of these errors can be recovered by the naive dictionary lookup.
...
We would like to thank the anonymous reviewers for their ... \todo{appropriate wording}

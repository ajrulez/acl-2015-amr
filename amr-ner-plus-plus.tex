...
The Abstract Meaning Representation (AMR) \cite{2013banarescu-amr} is a rich, graph-based language for expressing semantics over a broad domain.
% AMR is written as a directed, rooted graph merging propositional logic and neo-Davidsonian semantics (\todo{cite}).
The formalism is backed by a large data-labeling effort, and it holds
%parsing 
...
\reffig{glee} shows an example AMR for ``he gleefully ran to his dog Rover,'' and we give a brief introduction to AMR in \refsec{crash}.
...
A graphical explanation of our method. We represent the derivation process for \w{He gleefully ran to his dog Rover}. First the tokens in the sentence are labeled with derivation actions, then these actions are used to generate AMR subgraphs, which are then stitched together to form a coherent whole.}
%
...
The first step is \textit{concept identification}, which generates AMR nodes from text, and which we'll refer to as \textit{NER++} (\refsec{ner}). 
...
If a system is considering how to link the node \n{run-01} in \reffig{glee}, the verb-sense frame for ``run-01'' leaves very little uncertainty for what we could assign as an \w{ARG0} arc.
...
The primary contribution of this paper is a novel approach to the NER++ task, illustrated in \reffig{method}. We notice that the subgraphs aligned to lexical items can often be generated from a small set of \textit{generative actions} which generalize across tokens. For example, most verbs generate an AMR node corresponding to the verb sense of the appropriate PropBank frame -- e.g., \w{run} generates \n{run-01} in \reffig{glee}. This allows us to frame the NER++ task as the task of classifying one of a small number of \textit{actions} for each token, rather than choosing a specific AMR subgraph for every token in the sentence.

%The primary contribution of this paper is a method to largely delexicalize NER++, which leads to better performance and gracefully handles unseen tokens.
%Choosing from among a small set of \textit{generative actions} our system can derive an AMR subgraph from a span of tokens (see \reffig{method}).
...
%Although not all of our actions are deterministic and therefore we
%  expect some degradation in performance from errors in generating the correct 
%  AMR node given an action, we estimate our accuracy generating generating
%  correct AMR tokens to be approximately 77\%
...
% As an example of the distinction, let's briefly go over \reffig{glee}, the AMR for ``he gleefully ran to his dog Rover''. To produce this parse, the NER++ task will have to do a verb sense disambiguation on ``ran'' to get ``run-01'', and a lemmatization on ``gleefully'' to ``glee''. Then NER++ will have to recognize ``Rover'' is a name, and generate the subgraph (name :op1 ``Rover''). It will also have to recognize that ``to'' is a dropped preposition, and so doesn't get a node, and ``his'' is a coreferant that also doesn't get a node. Then ``dog'' and ``he'' will have to be generated directly as nodes. The SRL++ task is then responsible for linking together the output of the NER++ task to produce a fully-connected semantic graph.
...
%In fact, AMR will often expand single tokens into large subgraph elements, or ignore tokens completely.
...
\Fig{date.png}{0.25}{date}{AMR representation of the span \w{January 1, 2008}, an example of how AMR can represent structured data by creating additional nodes such as \n{date-entity} to signify the presence of special structure.}
...
To illustrate, a correct parse of the sentence in \reffig{method} requires lemmatization (\textit{gleefully} $\rightarrow$ \textit{glee}), word sense tagging (\textit{run} $\rightarrow$ \textit{run-01}), 
...
This paper's main contribution, presented in \refsec{nerplusplus}, is
  to make use of generative \textit{actions} to generate these subgraphs, rather than appealing
  to a memorized mapping.
%explores delexicalizing NER++ instead, see \refsec{nerplusplus}.

%Parsing to the AMR representation demands a rich NER system, word sense disambiguation, number normalization, time parsing, and many semantic nominalizations and part of speech translations, and within-sentence coreference. We refer to these ``low-level'' AMR tasks collectively as NER++. NER++ is the sub-task of generating the best AMR subgraphs (``subgraph'' is defined in \refsec{sub-chunks}) given the set of tokens $S$. This involves both partitioning the source text into spans that will be rendered as a single subgraph in AMR (e.g. ``run'', ``People's Republic of China'', ``January 1, 2008''), and then mapping each of those spans into a corresponding AMR subgraph of maximum likelihood.
...
Whereas NER++ produces subgraphs whose arcs encode domain-specific semantics
  (e.g., \textit{month}), 
%Unlike the domain-specific arcs within a single subgraph produced by NER++, 
  the arcs in SRL++ tend to have generally applicable semantics.
For example, the many arcs encode conventional semantic roles 
  (e.g., \e{ARG0} and \e{destination} in \reffig{method}),
  or a notion akin to syntactic dependencies (e.g., \e{mod} and \e{poss} in \reffig{method}).
...
%Given a perfect NER++ system for an AMR parser, there remains the task of noting the verb arguments, preposition sense actionging, and doing some augmented semantic dependency parsing in order to join the disjoint NER++ output into a single AMR parse. We call this task SRL++. SRL++ is the sub-task of taking as input the disjoint subgraphs generated by NER++, and adding the maximum likelihood set of arcs between the subgraphs in order to have a fully connected graph.
...
This serves as a reasonable baseline for word sense disambiguation, although of
  ...
  incorporated.
%look for the most similar PropBank frame and make that the title of the corresponding node.
...
%EDIT: Caveat about only applying to English.
It is worth observing at this point that our actions derive much of their power from the similarity 
  ...
  languages remains an open problem. %and so does not generalize to non-English AMR parsing.
...
% EDIT: added "alignment" to "alignment ambiguity" to clarify that the Action Reliability introduction
...
During the creation of training data for our classifier (\refsec{data}) from our aligner, 
...
In turn, in our aligner we bias alignments towards those which generating 
  more reliable action sequences as training data (see \refsec{alignment}).
...
The approach taken in \newcite{2014flanigan-amr} can be thought of as
...
% which will not break on unseen words.
...
%It's not always possible to derive an AMR subgraph directly from tokens at test 
...
%To implement a \textbf{DICT} class, we memorize a simple mapping from spans of text, like ``sailor'' to their corresponding most frequently seen AMR subgraphs in the training data, in this case \reffig{sailor}. At test time we can do a lookup in this dictionary for any element that gets labeled with a \textbf{DICT} action. Previous approaches have been the equivalent of labeling every node with the \textbf{DICT} action, so our reduction of its use is significant. \reftab{distro} gives the distribution of actions on the LDC2014T12 proxy training data, after our automatic alignment allows us to induce actions (see \refsec{data} for how this is done).
...
%At test time, given a sequence of input tokens, we do a simple classification of each token separately, to get a sequence labeling of our input tokens. Then for each token, we apply the behavior associated with the token label, and the resulting set of subgraphs is passed on to SRL++ for linking.
...
%EDIT: Added paragraph explaining jones2012semanics
Prior work in AMR and related formalisms include \newcite{jones2012semantics}, and \newcite{2014flanigan-amr}.
\newcite{jones2012semantics}, motivated by applications in Machine Translation, proposed a graphical semantic meaning representation that predates AMR, but is intimately related.
They propose a hyper-edge replacement grammar (HRG) approach to parsing into and out of this graphical semantic form.
\newcite{2014flanigan-amr} forms the basis of the approach of this paper.
...
%EDIT: Added paragraph about other AMR results since submission
%Since this paper was submitted for publication, more work has been published on AMR parsing.
\newcite{wangtransition} uses a transition based algorithm to transform dependency trees into AMR parses.
They achieve 64/62/63 P/R/F$_1$ with contributions roughly orthogonal to our own.
%  with lower precision and higher recall.
%  1 point improvement over the f-score of the JAMR-based\cite{2014flanigan-amr} system we demonstrate, 
...
%\todo{Find out the other published AMR paper in ACL this year}
...
on two corpora: the newswire section of LDC2014T12 and the split given in \newcite{2014flanigan-amr} of LDC2013E117, both consisting primarily of newswire.
...
%EDIT: Clarifying that this isn't another AMR corpus
% "prior work" -> "prior work on alignments", "corpus" -> "alignment dataset", "annotation scheme" -> "alignment annotation scheme"
...
%EDIT: Error analysis
We find the our informativeness-based alignment objective slightly improves end-to-end performance
...
%  \todo{this looks like a fraction of a point. Do we have the actual value?}
%  moving up from P=64, R=59, F1=61 to P=65, R=59, F1=62.
...
%EDIT: Added an acknowledgements section

...
On a hand-annotated dataset of 100 AMR parses from the LDC2014T12 corpus,\footnote{
    Our dataset would be publicly released upon acceptance
  }
...
The majority of the errors cluster in ambiguities between \textbf{NONE} and \textbf{DICT}, which are understandable since those are the two largest classes.
There is surprisingly little confusion between \textbf{NONE} and the other actions, given that \textbf{NONE} is the largest action class. We suspect this is because tokens in \textbf{NONE} have strong part of speech features, where for example prepositions are almost exclusively \textbf{NONE}.
